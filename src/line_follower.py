...
class LineFollower:
  ...
  def compute_error(self, cur_pose):
    ...
    car_forward_vec = np.array((np.cos(cur_pose[2]),np.sin(cur_pose[2])))
    while len(self.plan) > 0:
      # YOUR CODE HERE
      traj_point_vec = np.array((self.plan[0][0] - cur_pose[0], self.plan[0][1] - cur_pose[1]))
      if np.dot(car_forward_vec, traj_point_vec) < 0:
    ...
    # YOUR CODE HERE
    ref_pose = self.plan[goal_idx]
    car_left_vec = np.array(((np.cos((np.pi/2.0) + cur_pose[2])),(np.sin((np.pi/2.0) + cur_pose[2]))))
    error_vec = np.array((ref_pose[0] - cur_pose[0], ref_pose[1] - cur_pose[1]))
    # error_vec = error_vec/np.linalg.norm(error_vec) 
    translation_error = np.dot(car_left_vec, error_vec)
    # translation_error = -np.dot( error_vec, car_left_vec)
    ...
    # YOUR CODE HERE
    rotation_error = ref_pose[2] - cur_pose[2]
  ...
  def compute_steering_angle(self, error):
    ...
    if len(self.error_buff) != 0:
      previous_error = self.error_buff[-1]
      deriv_error = (error - previous_error[0]) / (now - previous_error[1])
    ...
    integ_error = 0
    while index > 0:
      duration = self.error_buff[index][1] - self.error_buff[index-1][1]
      # integ_error += self.error_buff[index][0] * duration
      integ_error += ((self.error_buff[index][0] + self.error_buff[index-1][0]) / 2) * duration
      index = index - 1
